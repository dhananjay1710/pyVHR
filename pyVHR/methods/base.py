...
class VHRMethod(metaclass=ABCMeta):
    ...
    def runOffline(self, **kwargs):
        ...
        bvpES = [] #bvp signal
        Redsig= [] #red ppg signal
        Greensig= [] #green ppg signal
        Bluesig= []
        ...
        while T <= endTime:
            ...
            #print(type(bvpChunk.data))
            #normalization
            bvpChunk.data=preprocessing.normalize(bvpChunk.data)
            bvpChunk.findPeaks(distance=None, height=None)
            
            red=RGBsig[1].reshape(-1,1)
            red=preprocessing.normalize(red)
            #transformer = RobustScaler().fit(red)
            #red=transformer.transform(red)
            
            green=RGBsig[2].reshape(-1,1)
            green=preprocessing.normalize(green)
            #ttransformer=RobustScaler().fit(green)
            #green=ttransformer.transform(green)
            
            blue=RGBsig[0].reshape(-1,1)
            blue=preprocessing.normalize(blue)
            #btransformer=RobustScaler().fit(blue)
            #blue=btransformer.transform(blue)
            #print(bvpChunk.data.size)
            # -- save the estimate
            bpmES.append(bvpChunk.bpm)
            timesES.append(T)
            bvpES.append(bvpChunk.data)
            Redsig.append(red)
            Greensig.append(green)
            Bluesig.append(blue)
        ...
        self.bvp =bvpES #np.array(bvpES)
        # TODO controllare se mettere o no il filtro seguente
        #self.bpm = self.bpm_time_filter(self.bpm, 3)
        self.times = np.array(timesES)
        self.red_sig=Redsig
        self.green_sig= Greensig
        self.blue_sig=Bluesig
        self.fps=fs
        return self.bpm, self.times, self.bvp, self.fps, self.red_sig, self.green_sig, self.blue_sig
